...
-----------------------------  Hoare Condition ----------------------------
-- This was added for the Hoare semantics portion of the coding project.
-- I realized that I needed to make a new condition without the broadcast
-- from the Mesa semantics. And honestly, it was much easier to split the
-- two conditions when I was trying to keep track of what I was doing with the 
-- different types of semantics.


  behavior hoareCondition
    -- Test implementation for the hoare semantics
...
-----------------------------  ThreadManager  ---------------------------------
  ...
  behavior ThreadManager
	...
	-- TEST IMPLEMENTATION	
	-- Placement holder so I don't get lost in code later.
	-- MATTHEW O'BRIEN
	...
	-- Both semantics are included here. I simply created a different condition
	-- type of the Hoare semantics. I didn't really implement any real extra
	-- test for this part. When I ran the original test function, everything still worked.
	-- Hopefully that was enough.
	-- I also did not complete the switch to Hoare semantics with the rest of the assignment
	-- I honestly got too excited about getting this to work, that I completely forgot about
	-- doing the rest of the assignment.
	-- Thankfully, it wasn't completely required. And it shouldn't be too difficult to implement
	-- them later on by switching around my While statements to If. 
	-- Sorry if this is a problem.
	
	--MESA Condition
	freedThread = new Condition
	freedThread.Init ()

	--Hoare Condition
	-- hoareFreedThread = new hoareCondition
	-- hoareFreedThread.Init ()
	...
	endMethod
 	...
 	-- TEST IMPLEMENTATION
	-- PLACE HOLDER SO I DON'T GET LOST LATER!


	var th: ptr to Thread = null    
	...
	-- Key part of the mesa semantics. 
	-- The while portion is the key of mesa.

	while freeList.IsEmpty ()
	  freedThread.Wait (& threadLock)
	endWhile
	...
	-- HOARE SEMANTICS!
	-- Again, no real extra test for the Hoare semantics. 
	-- I simply commented this portion out after I tested it,
	-- and everything still ran fine.

	--threadLock.Lock ()
	--if freeList.IsEmpty ()
	  --hoareFreedThread.Wait (& threadLock)
	--endIf
	...
	-- PLACE HOLDER TO NOT GET LOST LATER
	...
	-- Signal 
	freedThread.Signal (& threadLock)
	
	-- Exit critical.
	threadLock.Unlock ()

	-- HOARE SEMANTICS!
  	-- More Hoare semantic implementation.
	-- Commented out after it ran fine.
	-- Note: This portion of the code is still
	-- the exact same as the Mesa semantics.
	-- It only needed it's own section because of the 
	-- different type of condition created for Hoare.
	...
	--hoareFreedThread.Signal (& threadLock)
	--threadLock.Unlock ()
...
-- I uncommented this code, and it broke everything.
-- So I left it commented out because I don't understand.
...
-- Uncomment this code later...
-- More of the same as above. It broke everything.
...
-----------------------------  ProcessManager  ---------------------------------
  ...
  behavior ProcessManager
	...
	-- Create mutex and INIT the mutex.	
	...
	-- Initializing the two created conditions for a process ending and became free.
	aProcessBecameFree.Init ()
	aProcessDied.Init ()

	-- List for the process control block.
	freeList = new List [ProcessControlBlock]

	-- Initializing and setting status to free of the process table.
	-- Adding them to the end of free list.
	...
	-- Pointer for the process control block.
	
	var processPtr: ptr to ProcessControlBlock
	...
	while freeList.IsEmpty ()
	  aProcessBecameFree.Wait (& processLock)
	endWhile

	-- Taking the process off the free list.	
	processPtr = freeList.Remove ()
	
	-- Moving the nextpid down the list.
	processPtr.pid = nextPid
	nextPid = nextPid + 1
	
	-- Setting process to active.
	...
	-- Setting the process to free.
	p.status = FREE

	-- Adding the process to the end of the free list.
	freeList.AddToEnd (p)
	
	-- Signalling the process is freed.
	aProcessBecameFree.Signal (& processLock)
...
-----------------------------  FrameManager  ---------------------------------
  ...
  behavior FrameManager
	...
	-- TEST IMPLEMENTATION
	-- PLACE HOLDER AS TO NOT GET LOST LATER!

	-- PART 3 
	
	-- Setting up variables.
	var j, frameAddr, i: int
	  numberFramesAllocated: int = 0
	
	-- Critical
	frameManagerLock.Lock ()

	-- Waiting while the free frames and less then the frames needed
	-- While shows mesa semantics.

	while numberFreeFrames < numFramesNeeded
	  newFramesAvailable.Wait (& frameManagerLock)
	endWhile

	-- Moving through the frames needed
	-- Reducing frames as they are used, and 
	-- increasing the allocated frames.
	
	for i = 0 to numFramesNeeded - 1
	  j = framesInUse.FindZeroAndSet ()
	  frameAddr = PHYSICAL_ADDRESS_OF_FIRST_PAGE_FRAME + (j * PAGE_SIZE)
	  aPageTable.SetFrameAddr (i, frameAddr)
	  numberFreeFrames = numberFreeFrames - 1
	  numberFramesAllocated = numberFramesAllocated + 1
	endFor

	
	aPageTable.numberOfPages = numberFramesAllocated
	...
	-- Setting up variables
	var frameAddr, bitNumber, i: int
	  numFramesReturned: int = aPageTable.numberOfPages
	
	-- Critical
	frameManagerLock.Lock ()

	-- Freeing up some frames. 
...
-----------------------------  InitFirstProcess  ---------------------------------------
  ...
  -- First getanewthread, initialize and fork. Execute startuserprocess, 0 argument. 
  -- Return after fork.
  -- The pseudocode given from the PDF.

  -- Implementation Project 5 Matthew O'Brien
...
-----------------------------  StartUserProcess  --------------------------------------
  ...
  -- Using the Pseudocode from the PDF for implementation. Credit where credit is due.
  ...
  function StartUserProcess ()
    ...
    -- allocate a new PCB with getanewprocess, initialize myThread, and myprocess
    ...
    if openFile == null
      FatalError ("Executable file failure.")
    endIf

    -- creat logical address space and read executable into it.
    initPC = openFile.LoadExecutable(& pcb.addrSpace)
    if initPC == -1
      FatalError ("No Entry Point. Failure to start kernel.")
    endIf
    fileManager.Close(openFile)
    
    -- compute initial value for the user-level stack. set to logical address
    -- just past end of logical address space.
    ...
    --Final step. Jumps to user-level.
...
-----------------------------  Handle_Sys_Exit  ---------------------------------
  ...
  function Handle_Sys_Exit (returnStatus: int)
      ...
      -- The returnStatus will be saved so that it can be passed to a Sys_Join
...
-----------------------------  Handle_Sys_Shutdown  ---------------------------------
  ...
  function Handle_Sys_Shutdown ()
      ...
      -- Matthew O'Brien Task 1
      -- System Shutdown, stops the kernel.
      -- PDF Explanation:
      -- Function will cause an immediate shutdown of the Kernel. It will not return.
...
-----------------------------  Handle_Sys_Yield  ---------------------------------
  ...
  function Handle_Sys_Yield ()
      -- PDF Explanation
      -- Yields CPU to another process on ready list. Once process is scheduled
      -- again, function will return. Similar to a "nop"

      -- In the example output each of these Handle_Sys functions had an 
      -- printed output. I couldn't find anything in the PDF early on that
      -- suggested/required it. So I'm going to use what I saw in the example.
      -- Just in case. 
...
-----------------------------  Handle_Sys_Fork  ---------------------------------
  ...
  function Handle_Sys_Fork () returns int
      ...
      -- Parent returns the pid of the child. Child returns zero. Creates
      -- a copy of the parent as the child. It's a copy.

      --Following the example output print statement.  
...
-----------------------------  Handle_Sys_Join  ---------------------------------
  ...
  function Handle_Sys_Join (processID: int) returns int
      ...
      -- Wait until the process with given pid has stopped. 
      -- returnStatus passed by that process to Sys_Exit will be returned.
      -- If Sys_Exit other process executes first, returnStatus will be saved
      -- until either its parent executes Sys_Join naming that process'
      -- pid or until parent stops.

      -- Following example output print statments
...
-----------------------------  Handle_Sys_Exec  ---------------------------------
  ...
  function Handle_Sys_Exec (filename: ptr to array of char) returns int
      ...
      -- The explanation from the pdf is a massive one.
      -- Passed name of file, assumed to be executable, read into memory,
      -- overwriting entire address space of current process, OS will begin
      -- executing new process. Any open files in current remain open, and 
      -- unchanged in new process. Normal no return else return -1.

      -- Similar to the InitFirstProcess and StartUserProcess pseudo code
      -- which was given in the PDF.
      ...
      -- load requires empty addrspace object.
      -- given code from pdf
      newAddrSpace = new AddrSpace

      -- More initializations
      strBuffer = new array of char {MAX_STRING_SIZE of '\0'}
      oldIntStatus = SetInterruptsTo (ENABLED)
      pcb = currentThread.myProcess
      newAddrSpace.Init ()

      -- differs from pdf slightly. using the previous functions logic with pcb.
      -- pdf states using currentthread, but i've already set that to pcb.
      ...
      -- The following logic is from the previous functions of InitFirstProcess
      -- and StartUserProcess.
      ...
      -- Creates the logical address and read into it.
      ...
      -- Freeing up the frames
      frameManager.ReturnAllFrames (& pcb.addrSpace)

      -- Initialize the new address space for the executable load
      pcb.addrSpace = newAddrSpace

      -- Closed to ensure nothing becomes locked up.
      fileManager.Close (openFile)
      ...
      -- Setting "true" for isUserThread shows current thread is a userthread.
      currentThread.isUserThread = true

      -- Becomes a user thread.
      BecomeUserThread (initUserStackTop, initPC, initSystemStackTop asInteger)

      -- arbitrary return value from PDF
      return 3000
...
-----------------------------  Handle_Sys_Create  ---------------------------------
  ...
  function Handle_Sys_Create (filename: ptr to array of char) returns int
      ...
      -- Example print statements from PDF.
