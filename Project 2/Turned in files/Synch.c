...
-----------------------------  Mutex  ---------------------------------
  ...
  behavior Mutex
	...
	--Remember where you where at before this point
	oldIntStat = SetInterruptsTo (DISABLED)

	--Check to see if heldBy is pointing to a null, if so, set it to the currentThread
	if heldBy == null
	  heldBy = currentThread

	--Doesn't have the lock, going to wait in line
	elseIf !self.IsHeldByCurrentThread ()
	  waitingThreads.AddToEnd (currentThread)
	  currentThread.Sleep ()

	--Error check, something should have happened.
	else
	...
	--Remember where you were for interrupts
	oldIntStat = SetInterruptsTo (DISABLED)

	--Check to see if heldBy is pointing to null, ERROR
	if heldBy == null
	  FatalError ("Trying to unlock something that isn't even locked")

	--Unlock yourself, and move down the list of waiting threads
	elseIf self.IsHeldByCurrentThread ()
	  t = waitingThreads.Remove ()
	  heldBy = t
	  if t != null
	...
	--This is the final error check
	else
	  FatalError ("Lock doesn't match")
	endIf	
	
        --Rest to old values. IMPORTANT!
	oldIntStat = SetInterruptsTo (oldIntStat)
        endMethod
	...
	if StrEqual (heldBy.name, currentThread.name) 
	  return true
         -- FatalError ("Unimplemented method")
	endIf
