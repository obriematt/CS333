...
-----------------------------  ProducerConsumer  ---------------------------------
  ...
  var
    ...
    --Max sized buffer
    freeSpace: Semaphore = new Semaphore

    --Empty buffer to be added to
    fullSpace: Semaphore = new Semaphore

    --Mutex lock for control
    jonLock: Mutex = new Mutex    
  ...
  function ProducerConsumer ()
      ...
      --Initializing the buffers and lock.
      freeSpace.Init (BUFFER_SIZE)
      fullSpace.Init (0)
      jonLock.Init ()
  ...
  function Producer (myId: int)
	...
	-- Take off from the full buffer, and lock
	freeSpace.Down ()
	jonLock.Lock ()
	...
	--Add to the empty space and unlock
	fullSpace.Up ()
	jonLock.Unlock ()
  ...
  function Consumer (myId: int)
      ...
      while true
        -- Perform synchroniztion...
	-- Take from the empty space and lock
	fullSpace.Down ()
	jonLock.Lock ()
...
-----------------------------  Dining Philosophers  ---------------------------------
  ...
  class ForkMonitor
    ...
    fields
      ...
      --Lock for control, five conditions to see when a philo can eat
      datLock: Mutex
      philoEating: array [5] of Condition
  ...
  behavior ForkMonitor
    ...
    method Init ()
      ...
      -- Initializing the philoEating Condition as well
      var 
	i: int
    	status = new array of int {5 of THINKING}
	philoEating = new array [5] of Condition {5 of new Condition}

      for i = 0 to 4
	philoEating[i].Init()
      endFor
   
      --Initialize the mutex for control.
      datLock = new Mutex
      datLock.Init ()
      endMethod

    --Added this to look to the left and right of current philo.
    method checkNeighbors (p: int)
    var
	L: int = (p + 1) % 5
	R: int = (p - 1) % 5

    if status[L] != EATING && status[R] != EATING && status[p] == HUNGRY
	status[p] = EATING
	self.PrintAllStatus()
	philoEating[p].Signal (& datLock)
    endIf
    endMethod

   --Picking up those forks bruh
    method PickupForks (p: int)
      -- This method is called when philosopher 'p' is wants to eat.
      --Removed this when I realized it was easier to make a separate method
      --var
	--R: int = (p-1) % 5
	--L: int = (p+1) % 5 

      --Locking, and my philo is HUNGRY!
      datLock.Lock ()
      status[p] = HUNGRY

      --Printing the status of all the philos
      self.PrintAllStatus () 
      
      --Look left and right, start eating if you can   
      self.checkNeighbors (p) 
   
      --More replaced code made easier from the extra method.
      --if status[L] != EATING && status[R] != EATING && status[p] == HUNGRY
	--status[p] = EATING
	--self.PrintAllStatus ()
	--philoEating[p].Signal (& datLock)
      ...
      --If it turns out I'm not eating then wait.
      if status[p] != EATING
	philoEating[p].Wait (& datLock)
      endIf
	...
	R: int = (p - 1) % 5
      ...
      --Show what everyone is up to.
      self.PrintAllStatus ()

      --Look at my neighbor's neighbor and let them eat if allowed.
      self.checkNeighbors (L)
      self.checkNeighbors (R)
